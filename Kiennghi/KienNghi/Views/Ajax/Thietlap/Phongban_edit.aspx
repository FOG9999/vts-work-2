            ...
            <div class="row-fluid">
        		<div class="span12">
                    <div class="box box-color">
						<div class="box-title">
							<h3 >
								<i class="icon-reorder"></i> Cập nhật phòng ban
							</h3>
                            <ul class="tabs">
								<li class="active">
                        ...
                        <div class="box-content popup_info">
                            <form method="post" name="_form" id="_form" class="form-horizontal" onsubmit="return CapNhat();">
                                <% USER_PHONGBAN phong = (USER_PHONGBAN)ViewData["phong"]; %>
                                <div class="control-group">
							        <label for="textfield" class="control-label ">Tên phòng ban <i class="f-red">*</i></label>
							        <div class="controls">
                                        <input type="text" value="<%=Server.HtmlEncode(phong.CTEN) %>"  name="cTen" id="cTen" class="input-block-level"  onchange="kiemtrungten()" />
							        </div>
						        </div>
                                <div class="control-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        if ($("#iPhongBanCha").val() != 0) {
            $("#iDonVi").prop('disabled', true).trigger("liszt:updated");
        }
    });
    function CapNhat() {
        if ($("#cTen").val() == "") {
        ...
        $.post("/Thietlap/Ajax_Phongban_update", $("#_form").serialize(), function (ok) {
            if (ok == 1) {
                location.reload();
