create or replace PROCEDURE "PRC_KIENNGHI_MOICAPNHAT_CHANGE" 
(
        p_IUSER IN NUMBER,
      ...
      INNER JOIN
        (        
            ...
            (
                ...
                WHERE  (p_LSTNGUONKN = '' OR p_LSTNGUONKN IS NULL OR  KN_NGUONDON.INGUONDON IN (  Select Regexp_Substr(p_LSTNGUONKN,'[^,]+',1,Level) INGUONDON From Dual Connect By Regexp_Substr(p_LSTNGUONKN,'[^,]+',1,Level) Is Not Null))
...
--        AND (p_LSTNGUONKN IS NULL  OR  KN_NGUONDON.INGUONDON IN ( Select Regexp_Substr(p_LSTNGUONKN,'[^,]+',1,Level) INGUONDON From Dual Connect By Regexp_Substr(p_LSTNGUONKN,'[^,]+',1,Level) Is Not Null)) 
        ...
        AND (p_TUNGAY is null OR KN_KIENNGHI.DDATE >= p_TUNGAY)
        AND (p_DENNGAY is null OR KN_KIENNGHI.DDATE <= p_DENNGAY)
...
END PRC_KIENNGHI_MOICAPNHAT_CHANGE;