create or replace PROCEDURE RPT_KNTC_BAOCAO_CHUACOTRALOI_THEOLINHVUC_3D 
(
    P_IYEAR IN NUMBER,
    P_IMONTH IN NUMBER,
    P_TUNGAY IN DATE,
...
SELECT to_char( ROW_NUMBER() OVER (ORDER BY kntc_vanban.ivanban)) AS STT,
       ...
       TO_DATE(kntc_vanban.DNGAYBANHANH, 'DD/MM/YY') AS DNGAYBANNHANH,
       kntc_vanban.CSOVANBAN,
       CONCAT(CONCAT(trim(kntc_don.cnguoigui_ten), ', '),CONCAT(kntc_don.cnguoigui_diachi, CONCAT(', ',CONCAT(diaphuong2.cten,CONCAT(', ',diaphuong1.cten))))) AS CNGUOIGUITEN,
       kntc_don.CNOIDUNG,
       QUOCHOI_COQUAN.CTEN AS CCOQUANNHAN,
...
where (kntc_vanban.cloai = 'vanbandondocthuchien' OR kntc_vanban.cloai = 'chuyenxuly_noibo')
  ...
  AND (P_IYEAR = 0 OR P_IYEAR IS NULL OR EXTRACT(YEAR FROM kntc_vanban.dngaybanhanh) = P_IYEAR)
  AND (P_IMONTH = 0 OR P_IMONTH IS NULL OR EXTRACT(MONTH FROM kntc_vanban.dngaybanhanh) = P_IMONTH)
  ...
  AND (P_IDOITUONG = 0 OR P_IDOITUONG IS NULL OR kntc_don.idoituonggui = P_IDOITUONG)
--        AND kntc_vanban.ICOQUANNHAN != 429
  AND kntc_don.IDELETE = 0
;
END RPT_KNTC_BAOCAO_CHUACOTRALOI_THEOLINHVUC_3D;