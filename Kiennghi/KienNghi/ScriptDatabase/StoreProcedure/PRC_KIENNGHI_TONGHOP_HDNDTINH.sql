create or replace PROCEDURE "PRC_KIENNGHI_TONGHOP_HDNDTINH" (
    res out sys_refcursor,
    p_ITINHTRANG in NUMBER, /* DEFAULT: -1*/   
    p_ILINHVUC IN NVARCHAR2,
...
Open res for
  ...
  TBL1 AS(
    ...
    KIENNGHI AS(
      ...
      COALESCE(KN_KIENNGHI.ILINHVUC,0) AS ID_LINHVUC_KIENNGHI, KN_KIENNGHI.IDIAPHUONG0 as IDIAPHUONG0,
      LINHVUC_COQUAN.CTEN AS TEN_LINHVUC_KIENNGHI, QUOCHOI_COQUAN.CTEN AS TEN_DONVITIEPNHAN_KIENNGHI, 
      KN_KIENNGHI.ITRUOCKYHOP, KN_KIENNGHI.CNOIDUNG,
...
--          UPPER(KN_KIENNGHI.CNOIDUNG) LIKE '%' || UPPER(p_CNOIDUNGKIENNGHI) || '%' AND
--          (p_CNOIDUNGKIENNGHI = '' OR p_CNOIDUNGKIENNGHI IS NULL OR UPPER(KN_KIENNGHI.CNOIDUNG) LIKE '%' || UPPER(p_CNOIDUNGKIENNGHI) || '%') AND
          (p_DTUNGAY is null OR KN_KIENNGHI.DDATE >= p_DTUNGAY) AND
          (p_DDENNGAY is null OR KN_KIENNGHI.DDATE <= p_DDENNGAY) AND
          (p_INGUONKIENNGHI='0' OR p_INGUONKIENNGHI IS NULL  OR  KN_NGUONDON.INGUONDON IN ( Select Regexp_Substr(p_INGUONKIENNGHI,'[^,]+',1,Level) INGUONDON From Dual Connect By Regexp_Substr(p_INGUONKIENNGHI,'[^,]+',1,Level) Is Not Null)) AND
          (p_IDONVITIEPNHAN = 0 OR p_IDONVITIEPNHAN IS NULL OR KN_KIENNGHI.IDONVITIEPNHAN = p_IDONVITIEPNHAN) AND
          (p_ITHAMQUYENDONVI= 0 OR KN_KIENNGHI.ITHAMQUYENDONVI=p_ITHAMQUYENDONVI) AND KN_KIENNGHI.ID_GOP<=0 AND
          (p_ILINHVUC='' OR p_ILINHVUC IS NULL 
                        OR LINHVUC_COQUAN.ILINHVUC IN (Select Regexp_Substr(p_ILINHVUC,'[^,]+',1,Level) From Dual Connect By Regexp_Substr(p_ILINHVUC,'[^,]+',1,Level) Is Not Null)
                        OR LINHVUC_COQUAN.IPARENT IN (Select Regexp_Substr(p_ILINHVUC,'[^,]+',1,Level) From Dual Connect By Regexp_Substr(p_ILINHVUC,'[^,]+',1,Level) Is Not Null)) 
           ...
           AND (KN_KIENNGHI.IDIAPHUONG0 IN (select regexp_substr(p_LIST_HUYEN_XA_TP,'[^,]+', 1, level)
                        from dual
...
END PRC_KIENNGHI_TONGHOP_HDNDTINH;