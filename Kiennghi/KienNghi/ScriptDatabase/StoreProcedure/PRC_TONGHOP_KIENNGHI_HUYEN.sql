create or replace PROCEDURE "PRC_TONGHOP_KIENNGHI_HUYEN" (res out sys_refcursor,  p_ITINHTRANG in NUMBER, /* DEFAULT: -1*/ 
--                                                               p_ILINHVUC IN NUMBER, /* DEFAULT: ''*/
                                                               p_CMAKIENNGHI IN NVARCHAR2, /* DEFAULT: ''*/
                                                               ...
                                                               p_IDONVITIEPNHAN in NUMBER,/* DEFAULT: -1*/
...
--                AND (p_IKYHOP=0 OR KN_KIENNGHI.IKYHOP=p_IKYHOP)
    ...
    TBL2 as( select  KN_GOP.ID_KIENNGHI_PARENT_GOP AS ID_KIENNGHI_PARENT_GOP,
     ...
     KIENNGHI AS(
      ...
      LINHVUC_COQUAN.CTEN AS TEN_LINHVUC_KIENNGHI,QUOCHOI_COQUAN.CTEN AS TEN_DONVITIEPNHAN_KIENNGHI, KN_KIENNGHI.CNOIDUNG,
      ...
      WHERE 1=1 AND (p_IUSER = 0 OR KN_KIENNGHI.IUSER = p_IUSER) AND
          ...
          (p_ITHAMQUYENDONVI=0 OR KN_KIENNGHI.ITHAMQUYENDONVI=p_ITHAMQUYENDONVI) AND KN_KIENNGHI.ID_GOP<=0
            ...
            AND  (KN_KIENNGHI.IDIAPHUONG0 IN (select regexp_substr(p_LIST_HUYEN_TP,'[^,]+', 1, level)
                        from dual
                        connect BY regexp_substr(p_LIST_HUYEN_TP, '[^,]+', 1, level)
                        is not null) OR p_LIST_HUYEN_TP = '' OR p_LIST_HUYEN_TP is null)      
            AND UPPER(KN_KIENNGHI.CMAKIENNGHI) LIKE '%' || UPPER(TRIM(p_CMAKIENNGHI)) || '%'
            AND (p_DTUNGAY is null OR KN_KIENNGHI.DDATE >= p_DTUNGAY)
            AND (p_DDENNGAY is null OR KN_KIENNGHI.DDATE <= p_DDENNGAY)
            AND (p_IDONVITIEPNHAN is null OR p_IDONVITIEPNHAN = 0 OR KN_KIENNGHI.IDONVITIEPNHAN = p_IDONVITIEPNHAN)
    ...
    LEFT JOIN TRACKING ON TRACKING.IKIENNGHI=KIENNGHI.IKIENNGHI AND TRACKING.ITONGHOP=TBL1.ITONGHOP
		...
		WHERE KIENNGHI.ITINHTRANG = 1 AND QUOCHOI_COQUAN.CTYPE = 'HUYEN'
