...
// Namespace được xây dựng bới Hoàng Dũng (dungh) thuộc buyt thuộc trung tâm giải pháp và phát triển phần mềm viễn thông VIETTEL-ICT
// Được phát triển config theo giải pháp của Microsoft dành cho bộ sql helper 
namespace DataAccess
{
    ...
    public sealed class OracleHelper
    {
        ...
        private static void AssignParameterValues(OracleParameter[] commandParameters, object[] parameterValues)
        {
            ...
            if (lenghtinputValues != parameterValues.Length)
            {
            ...
            {
                if (commandParameters[i].Direction == ParameterDirection.Input || commandParameters[i].Direction == ParameterDirection.InputOutput)
                { 
                    commandParameters[i].Value = parameterValues[i]; 
                }
                else if (commandParameters[i].Direction == ParameterDirection.Output || commandParameters[i].Direction == ParameterDirection.ReturnValue)
                {
        ...
        public static DataSet ExecuteDataset(string connectionString, CommandType commandType, string commandText, params OracleParameter[] commandParameters)
        {
            ...
            {
                cn.Open();
                commandParameters = OracleHelperParameterCache.GetSpParameterSet(connectionString, commandText);
                //call the overload that takes a connection in place of the connection string
        ...
        private static OracleDataReader ExecuteReader(out object outputparameterValue, OracleConnection connection, OracleTransaction transaction, CommandType commandType, string commandText, OracleParameter[] commandParameters, OracleConnectionOwnership connectionOwnership)
        {
            //create a command and prepare it for execution
            OracleCommand cmd = new OracleCommand();
            PrepareCommand(cmd, connection, transaction, commandType, commandText, commandParameters);
            ...
            if (connectionOwnership == OracleConnectionOwnership.External)
            {
                ...
                foreach (OracleParameter item in commandParameters)
                {
                    if (item.DbType == DbType.Decimal && item.Value != null && (item.Direction == ParameterDirection.InputOutput || item.Direction == ParameterDirection.Output))
                    {
            ...
            outputparameterValue = returnvalue;
            return (OracleDataReader)dr;
        }

        private static OracleDataReader ExecuteReader(out List<object> ListoutputparameterValues, OracleConnection connection, OracleTransaction transaction, CommandType commandType, string commandText, OracleParameter[] commandParameters, OracleConnectionOwnership connectionOwnership)
        {
            //create a command and prepare it for execution
            OracleCommand cmd = new OracleCommand();
            PrepareCommand(cmd, connection, transaction, commandType, commandText, commandParameters);
            ...
            if (connectionOwnership == OracleConnectionOwnership.External)
            {
                ...
                foreach (OracleParameter item in commandParameters)
                {
                    if (item.DbType == DbType.Decimal && item.Value != null && (item.Direction == ParameterDirection.InputOutput || item.Direction == ParameterDirection.Output))
                    {
                        ...
                        objectreturn = item.Value;
                        list.Add(objectreturn);
                    }
                    if (item.DbType == DbType.Object && (item.Direction == ParameterDirection.InputOutput || item.Direction == ParameterDirection.Output))
                    {
                        if (countresultindex != 0)
                        {
                            for (int i = 0; i < countresultindex; i++)
                                dr.NextResult();
                            DataTable objectreturn = new DataTable();
                            objectreturn.Load(dr);
                            list.Add(objectreturn);
                        }
                        countresultindex++;
                    }
            ...
            {
                dr = cmd.ExecuteReader((CommandBehavior)((int)CommandBehavior.Default));                
                foreach (OracleParameter item in commandParameters)
                {
                    if (item.DbType == DbType.Decimal && item.Value != null && (item.Direction == ParameterDirection.InputOutput || item.Direction == ParameterDirection.Output))
                    {
            ...
            ListoutputparameterValues = list;
            return (OracleDataReader)dr;
        }
        ...
        public static OracleDataReader ExecuteReader(out object outputparameterValue , string connectionString, CommandType commandType, string commandText)
        {
            OracleParameter[] commandParameters = OracleHelperParameterCache.GetSpParameterSet(connectionString, commandText);
            //pass through the call providing null for the set of OracleParameters
            return ExecuteReader(out outputparameterValue , connectionString, commandType, commandText, commandParameters);
        }

        public static OracleDataReader ExecuteReader(out List<object> ListoutputparameterValues, string connectionString, CommandType commandType, string commandText)
        {
            OracleParameter[] commandParameters = OracleHelperParameterCache.GetSpParameterSet(connectionString, commandText);
            //pass through the call providing null for the set of OracleParameters
            return ExecuteReader(out ListoutputparameterValues, connectionString, commandType, commandText, commandParameters);
        }
        ...
        public static OracleDataReader ExecuteReader(out object outputparameterValue , string connectionString, CommandType commandType, string commandText, params OracleParameter[] commandParameters)
        {
            //create & open an OraclebConnection
            OracleConnection cn = new OracleConnection(connectionString);
            cn.Open();
            ...
            try
            {
                ...
                //call the private overload that takes an internally owned connection in place of the connection string
                return ExecuteReader(out outputparameterValue , cn, null, commandType, commandText, commandParameters, OracleConnectionOwnership.Internal);
            }
            catch
            {
                //if we fail to return the OracleDataReader, we need to close the connection ourselves
        ...
        public static OracleDataReader ExecuteReader(out List<object> outputparameterValue, string connectionString, CommandType commandType, string commandText, params OracleParameter[] commandParameters)
        {
            //create & open an OraclebConnection
            OracleConnection cn = new OracleConnection(connectionString);
            cn.Open();
        ...
        public static OracleDataReader ExecuteReader(string connectionString, string spName, params object[] parameterValues)
        {
            ...
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                //assign the provided values to these parameters based on parameter order
                AssignParameterValues(commandParameters, parameterValues);
                //OracleDataReader s = ExecuteReader(connectionString, CommandType.StoredProcedure, spName, commandParameters);
                //call the overload that takes an array of OracleParameters
        ...
        public static OracleDataReader ExecuteReader(out object outputparameterValue, string connectionString, string spName, params object[] parameterValues)
        {
            //if we got parameter values, we need to figure out where they go
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                //OracleDataReader s = ExecuteReader(connectionString, CommandType.StoredProcedure, spName, commandParameters);
                //call the overload that takes an array of OracleParameters
                return ExecuteReader(out outputparameterValue , connectionString, CommandType.StoredProcedure, spName, commandParameters);

            }
            //otherwise we can just call the SP without params
            else
            {
                return ExecuteReader(out outputparameterValue , connectionString, CommandType.StoredProcedure, spName);
            }                      
        }

        public static OracleDataReader ExecuteReader(out List<object> ListoutputparameterValues, string connectionString, string spName, params object[] parameterValues)
        {
            //if we got parameter values, we need to figure out where they go
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                //OracleDataReader s = ExecuteReader(connectionString, CommandType.StoredProcedure, spName, commandParameters);
                //call the overload that takes an array of OracleParameters
                return ExecuteReader(out ListoutputparameterValues, connectionString, CommandType.StoredProcedure, spName, commandParameters);

            }
            //otherwise we can just call the SP without params
            else
            {
                return ExecuteReader(out ListoutputparameterValues, connectionString, CommandType.StoredProcedure, spName);
            }
        }
    ...
    public sealed class OracleHelperParameterCache
    {
        ...
        /// </summary>
        /// <remarks>
        /// This method will query the database for this ExecuteReaderinformation, and then store it in a cache for future requests.
        /// </remarks>
        /// <param name="connectionString">a valid connection string for an OracleConnection</param>
        /// <param name="spName">the name of the stored procedure</param>
