...
namespace VIETTEL.Models
{
    public class HamChung
    {
        ...
        /// <returns>Dat</returns>
        public static DataTable SelectDistinct(string TableName, DataTable SourceTable, string FieldName, String sFieldAdd, String sField_DK = "", String sField_DK_Value = "", String strSort = "", String MaND = "")
        {
        ...
        private static String LayMoTa(DataRow dr, String sField_DK, String sField_DK_Value = "")
        {
            ...
            {
                String SQL = "SELECT sMoTa FROM NS_MucLucNganSach WHERE iTrangThai=1 AND sLNS=@sLNS AND iNamLamViec = @iNamLamViec";
                SqlCommand cmd = new SqlCommand(SQL);
                cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        public static string RenderPartialViewToStringLoad(string ViewName, object Model, Microsoft.AspNetCore.Mvc.Controller controller)
        {
        ...
        public static DataTable getSLNS()
        {
            int iNamLamViec = Convert.ToInt32(ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name));
            String SQL = @"select sLNS, sLNS + ' - ' + sMoTa as TenLNS FROM NS_MucLucNganSach WHERE iNamLamViec=@iNamLamViec AND iTrangThai=1 AND sL='' ORDER BY sLNS";
            SqlCommand cmd = new SqlCommand(SQL);
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        public static DataTable GetLoaiKhoan(string lns)
        {
            var sql = @"
select TOP(1) sXauNoiMa, sLNS, sL, sK, sM, sTM, sTTM, sNG, sTNG, sMoTa from NS_MucLucNganSach 
where 
    iTrangThai = 1 AND
    inamlamviec = @inamlamviec AND
    sLNS = @sLNS AND sL != '' AND SK != ''
order by sLNS, sL, sk, sM, sTM, sTTM";
        ...
        public static string GetLoaiKhoanText(string lns)
        {
            var dt = GetLoaiKhoan(lns);
            if (dt == null || dt.Rows.Count == 0)
            {
                return string.Empty;
            }
