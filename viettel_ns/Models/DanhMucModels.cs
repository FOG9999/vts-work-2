...
namespace VIETTEL.Models
{
    public class DanhMucModels
    {
        ...
        {
            DataTable dt;
            SqlCommand cmd = new SqlCommand("SELECT sLNS, sLNS +' - '+ sMoTa AS TenHT FROM NS_MucLucNganSach WHERE LEN(sLNS)=7 AND iNamLamViec=@iNamLamViec AND sL = '' ORDER By sXauNoiMa");
            String iNamLamViec=ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name);
        ...
        public static DataTable NS_PBLoaiNganSach(Boolean All = false, String sLNS = null)
        {
            DataTable dt;
            SqlCommand cmd = new SqlCommand();
            var sql = @"select sLNS, TenHT = ISNULL(TenHT,sLNS + ' - ')
                                              from
                                                    (
                                                    select  Distinct sLNS 
                                                    from    NS_MucLucNganSach 
                                                    where   LEN(sLNS) = 7 and sL = '' ) all_lns

                                                    left join

                                                    (
                                                    select  sLNS as id, sLNS + ' - ' + sMoTa AS TenHT 
                                                    from    NS_MucLucNganSach 
                                                    where   LEN(sLNS) = 7 and iNamLamViec = @iNamLamViec and sL = '')  lns_nn

                                                    on all_lns.sLNS = lns_nn.id
                                               where @@where";
            if (String.IsNullOrEmpty(sLNS) == false)
            {
                sql = sql.Replace("@@where", "sLNS like @sLNS");
        ...
        {
            SqlCommand cmd = new SqlCommand();
            string sql = "SELECT sLNS, sLNS +' - '+ sMoTa AS TenHT FROM NS_MucLucNganSach WHERE LEN(sLNS)=7 AND iNamLamViec=@iNamLamViec AND sL = ''";
            if (String.IsNullOrEmpty(sLNS) == false)
            {
        ...
        {
            DataTable dt;
            SqlCommand cmd = new SqlCommand("SELECT sLNS, sLNS +' - '+ sMoTa AS TenHT FROM NS_MucLucNganSach WHERE LEN(sLNS)=7   AND A.iNamLamViec=@iNamLamViec  AND sLNS LIKE '2%' AND sL = '' ORDER By sLNS");
            dt = Connection.GetDataTable(cmd);
            String iNamLamViec = ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name);
            if (Convert.ToInt32(iNamLamViec) < 2018) iNamLamViec = "2017";
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        {
            DataTable dt;
            String SQL = "SELECT A.sLNS, A.sLNS +' - '+ A.sMoTa AS TenHT FROM NS_MucLucNganSach as A INNER JOIN NS_PhongBan_LoaiNganSach AS B ON A.sLNS = B.sLNS WHERE B.iID_MaPhongBan=@iID_MaPhongBan AND B.iTrangThai=1 AND LEN(A.sLNS)=7   AND A.iNamLamViec=@iNamLamViec  AND SUBSTRING(A.sLNS,1,1) <>'8' AND A.sL = '' ORDER By A.sLNS";
        ...
        public static DataTable NS_LoaiNganSachFull(String iID_MaPhongBan)
        {
            ...
            DataTable dt;
            String SQL = "SELECT A.sLNS, A.sLNS +' - '+ A.sMoTa AS TenHT FROM NS_MucLucNganSach as A INNER JOIN NS_PhongBan_LoaiNganSach AS B ON A.sLNS = B.sLNS WHERE B.iID_MaPhongBan=@iID_MaPhongBan AND B.iTrangThai=1 AND LEN(A.sLNS)=7  AND A.iNamLamViec=@iNamLamViec AND A.sL = '' ORDER By A.sLNS";
            SqlCommand cmd = new SqlCommand(SQL);
            String iNamLamViec = ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name);
            if (Convert.ToInt32(iNamLamViec) < 2018) iNamLamViec = "2017";
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        {
            DataTable dt;
            String SQL = "SELECT A.sLNS, A.sLNS +' - '+ A.sMoTa AS TenHT FROM NS_MucLucNganSach as A INNER JOIN NS_PhongBan_LoaiNganSach AS B ON A.sLNS = B.sLNS WHERE B.iID_MaPhongBan=@iID_MaPhongBan AND B.iTrangThai=1 AND LEN(A.sLNS)=7 AND A.sLNS LIKE '1%' AND A.sLNS <> '1020000' AND A.iNamLamViec=@iNamLamViec AND A.sL = '' ORDER By A.sLNS";
            SqlCommand cmd = new SqlCommand(SQL);
            String iNamLamViec = ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name);
            if (Convert.ToInt32(iNamLamViec) < 2018) iNamLamViec = "2017";
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        {
            DataTable dt;
            String SQL = "SELECT A.sLNS, A.sLNS +' - '+ A.sMoTa AS TenHT FROM NS_MucLucNganSach as A INNER JOIN NS_PhongBan_LoaiNganSach AS B ON A.sLNS = B.sLNS WHERE B.iID_MaPhongBan=@iID_MaPhongBan AND B.iTrangThai=1 AND LEN(A.sLNS)=7 AND (A.sLNS LIKE '2%' OR A.sLNS LIKE '3%') AND A.iNamLamViec=@iNamLamViec AND A.sL = '' ORDER By A.sLNS";
            SqlCommand cmd = new SqlCommand(SQL);
            String iNamLamViec = ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name);
            if (Convert.ToInt32(iNamLamViec) < 2018) iNamLamViec = "2017";
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        {
            DataTable dt;
            SqlCommand cmd = new SqlCommand("SELECT sLNS, sLNS +' - '+ sMoTa AS TenHT FROM NS_MucLucNganSach WHERE LEN(sLNS)=7 AND LEFT(sLNS,1) <> 2 AND sLNS <> '1010000' AND A.iNamLamViec=@iNamLamViec AND sL = '' ORDER By sLNS");
            dt = Connection.GetDataTable(cmd);
            String iNamLamViec = ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name);
            if (Convert.ToInt32(iNamLamViec) < 2018) iNamLamViec = "2017";
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        {
            DataTable dt;
            String SQL = "SELECT A.sLNS as sLNS, A.sLNS +' - '+ A.sMoTa AS TenHT FROM NS_MucLucNganSach as A INNER JOIN NS_PhongBan_LoaiNganSach AS B ON A.sLNS = B.sLNS WHERE B.iID_MaPhongBan=@iID_MaPhongBan AND B.iTrangThai=1 AND LEN(A.sLNS)=7 AND A.iNamLamViec=@iNamLamViec AND LEFT(A.sLNS,1) <> 2 AND A.sLNS <> '1010000' AND A.sL = '' ORDER By A.sLNS";
            SqlCommand cmd = new SqlCommand(SQL);
            String iNamLamViec = ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name);
            if (Convert.ToInt32(iNamLamViec) < 2018) iNamLamViec = "2017";
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        {
            DataTable dt;
            String SQL = "SELECT A.sLNS as sLNS, A.sLNS +' - '+ A.sMoTa AS TenHT FROM NS_MucLucNganSach as A INNER JOIN NS_PhongBan_LoaiNganSach AS B ON A.sLNS = B.sLNS WHERE B.iID_MaPhongBan=@iID_MaPhongBan AND B.iTrangThai=1 AND LEN(A.sLNS)=7  AND A.iNamLamViec=@iNamLamViec AND A.sLNS <> '1010000' AND A.sL = '' ORDER By A.sLNS";
            SqlCommand cmd = new SqlCommand(SQL);
            String iNamLamViec = ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name);
            if (Convert.ToInt32(iNamLamViec) < 2018) iNamLamViec = "2017";
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        {
            DataTable dt;
            String SQL = "SELECT A.sLNS as sLNS, A.sLNS +' - '+ A.sMoTa AS TenHT FROM NS_MucLucNganSach as A INNER JOIN NS_PhongBan_LoaiNganSach AS B ON A.sLNS = B.sLNS WHERE B.iID_MaPhongBan=@iID_MaPhongBan AND B.iTrangThai=1 AND LEN(A.sLNS)=7  AND A.iNamLamViec=@iNamLamViec AND SUBSTRING(A.sLNS,1,1)<>'8' AND A.sL = '' ORDER By A.sLNS";
            SqlCommand cmd = new SqlCommand(SQL);
            String iNamLamViec = ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name);
            if (Convert.ToInt32(iNamLamViec) < 2018) iNamLamViec = "2017";
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
...
WHERE iTrangThai=1 AND iID_MaLoaiDanhMuc=(SELECT iID_MaLoaiDanhMuc FROM DC_LoaiDanhMuc WHERE iTrangThai=1 AND sTenBang='{0}') ORDER BY iSTT", TenLoaiDanhMuc);
        ...
        public static DataTable DT_Nam(Boolean CoHangChon = true, string sTenHangChon = "-- Bạn chọn năm --")
        {
            ...
            int NamMin = NamLamViec - 10;                
            int NamMax = NamLamViec + 10;
        ...
        public static DataTable DT_DoiTuong(Boolean ThemDongTieuDe = false, String sDongTieuDe = "--- Chọn tất cả ---")
        {
            ...
            R6["MaDT"] = "6";
            R6["TenDT"] = "Trợ lý B11";
            dt.Rows.InsertAt(R6, 6);

            DataRow R7 = dt.NewRow();
            R7["MaDT"] = "7";
            R7["TenDT"] = "Trợ lý B2";
            dt.Rows.InsertAt(R7, 7);

            DataRow R8 = dt.NewRow();
            R8["MaDT"] = "8";
            R8["TenDT"] = "Trợ lý tổng hợp B2";
            dt.Rows.InsertAt(R8, 8);

            DataRow R9 = dt.NewRow();
            R9["MaDT"] = "9";
            R9["TenDT"] = "Thủ trưởng B2";
            dt.Rows.InsertAt(R9, 9);

            DataRow R10 = dt.NewRow();
            R10["MaDT"] = "10";
            R10["TenDT"] = "Trợ lý tổng hợp b";
            dt.Rows.InsertAt(R10, 10);

            if (ThemDongTieuDe)
            {
                DataRow R11 = dt.NewRow();
                R11["MaDT"] = Guid.Empty;
                R11["TenDT"] = sDongTieuDe;
                dt.Rows.InsertAt(R11, 0);
            }
            return dt;
        }
