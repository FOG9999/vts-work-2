...
namespace VIETTEL.Models.DuToanBS
{
    public partial class DuToanBSChungTuChiTietModels
    {
        ...
        public static DataTable LayChungTuChiTietPhanCap2(string maChungTu, Dictionary<string, string> dicGiaTriTimKiem, string MaND, string sLNS, string sXauNoiMa, string iKyThuat, string MaLoai)
        {
            DataTable vR;
            ...
            if (iKyThuat == "1")
            {
                ...
                sql = String.Format(@"  select ct.* 
                                        from (
                                                ...
                                                ,rNgay,rSoNguoi,rChiTaiKhoBac,rTonKho,rTuChi,rChiTaiNganh,rChiTapTrung,rHangNhap,rHangMua,rHienVat,rDuPhong,rPhanCap,sGhiChu,iNamLamViec
                                            ...
                                            LEFT JOIN NS_DonVi AS dv ON dv.iID_MaDonVi = ct.iID_MaDonVi and dv.iNamLamViec_DonVi = ct.iNamLamViec

                                        ORDER BY sM,sTM,sTTM,sNG,dv.iSTT", dk);
                cmd.CommandText = sql;
            ...
            else
            {
                ...
                sql = String.Format("select ct.* from (SELECT *,sTenDonVi as sTenDonVi_BaoDam FROM DTBS_ChungTuChiTiet_PhanCap WHERE {0}) as ct LEFT JOIN NS_DonVi AS dv ON dv.iID_MaDonVi = ct.iID_MaDonVi and dv.iNamLamViec_DonVi = ct.iNamLamViec ORDER BY ct.sXauNoiMa,dv.iSTT", dk);
                cmd.CommandText = sql;
