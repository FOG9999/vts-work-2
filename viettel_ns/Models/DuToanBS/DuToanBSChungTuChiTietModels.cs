...
{
    public partial class DuToanBSChungTuChiTietModels
    {
        ...
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand("SELECT * FROM DTBS_ChungTu WHERE iTrangThai <> 0 AND iID_MaChungTu=@iID_MaChungTu");
            cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);
            vR = Connection.GetDataTable(cmd);
            cmd.Dispose();
            return vR;
        ...
        {
            DataTable dtResult, dtChungTu;
            string sql;
            string dk = "";
            SqlCommand cmd;
            DataTable dt = NganSach_HamChungModels.DSLNSCuaPhongBan(maND);

            dtChungTu = GetChungTu(maChungTu);
            ...
            if (arrLNS.Length > 0)
            {
                ...
                bool checkTL11_02 = LuongCongViecModel.KiemTra_TroLyPB11_02(maND);
                if (check == false && bTrolyTHCuc == false && checkTL11_02 == false)
                {
                    if (arrLNS[0].Equals("1040100"))
                    {
                        String DK = "";
                        DK += " AND sMaNguoiQuanLy like '%" + maND + "%'";
                        String SQL = String.Format(@"SELECT iID_MaNganhMLNS FROM NS_MucLucNganSach_Nganh
            WHERE iTrangThai=1 and iNamLamViec = @iNamLamViec {0}", DK);
                        ...
                        for (int i = dtResult.Rows.Count - 1; i >= 0; i--)
                        {
                            ...
                            {
                                if (Convert.ToString(dtResult.Rows[i]["sNG"]) == arrMaNganh[j] || Convert.ToBoolean(dtResult.Rows[i]["bLaHangCha"]) == true)
                                {
                                    bXoa = false;
                                    break;
                                }
                        ...
                        {
                            bool bXoa = true;
                            if (Convert.ToBoolean(dtResult.Rows[i]["bLaHangCha"]))
                            {
                                String iID_MaMucLucNganSach = Convert.ToString(dtResult.Rows[i]["iID_MaMucLucNganSach"]);
                                for (int j = i + 1; j < dtResult.Rows.Count; j++)
                                {
                                    if (iID_MaMucLucNganSach == Convert.ToString(dtResult.Rows[j]["iID_MaMucLucNganSach_Cha"]))
                                    {
                                        bXoa = false;
                                        break;
                                    }
        ...
        public static DataTable LayChungTuChiTietPhanCap(string maChungTu, Dictionary<string, string> dicGiaTriTimKiem, string MaND, string sLNS, string sXauNoiMa, string iKyThuat, string MaLoai)
        {
            ...
            if (iKyThuat == "1")
            {
                ...
                if (MaLoai == "2")
                {
                    dk = String.Format("iTrangThai=1 AND iNamLamViec = @iNamLamViec AND sLNS LIKE '{0}%' AND sXauNoiMa LIKE '%{1}%'", sLNS, sXauNoiMa);
                }
                else
                {
                    dk = String.Format("iTrangThai=1 AND iNamLamViec = @iNamLamViec AND sLNS LIKE '{0}%' AND sXauNoiMa LIKE '%{1}%'", sLNS, sXauNoiMa);
                }
                int iNamLamViec = Convert.ToInt32(ReportModels.LayNamLamViec(HttpContextHelper.Current.User.Identity.Name));
        ...
        public static DataTable LayChungTuChiTietLan2(String maChungTu, Dictionary<string, string> dicGiaTriTimKiem, string maND, string sLNS)
        {
            ...
            string iNamLamViec = Convert.ToString(dt.Rows[0]["iNamLamViec"]);
            cmd = new SqlCommand();
            ...
            if (vR.Rows.Count == 0)
            {
                ...
                else
                {
                    ...
                    SQL = String.Format("SELECT TOP 1 iID_MaNganhMLNS FROM NS_MucLucNganSach_Nganh WHERE iNamLamViec = @iNamLamViec AND iTrangThai=1 AND iID_MaNganh=@iID_MaNganh");
                    cmd = new SqlCommand(SQL);
            ...
            cmd = new SqlCommand();
            SQL = String.Format(@"SELECT iID_MaMucLucNganSach,iID_MaMucLucNganSach_Cha,bLaHangCha,sXauNoiMa,sTenCongTrinh,sLNS,sL,sK,sM,sTM,sTTM,sNG,sTNG,sMoTa,iID_MaChungTuChiTiet,iID_MaDonVi,sTenDonVi,sTenDonVi as sTenDonVi_BaoDam,iID_MaPhongBanDich,iID_MaPhongBan
                                ,rNgay,rSoNguoi,rChiTaiKhoBac,rTonKho,rTuChi,rChiTaiNganh,rChiTapTrung,rHangNhap,rHangMua,rHienVat,rDuPhong,rPhanCap,sGhiChu,sMaCongTrinh
                                ,brTuChi,brHienVat,brHangNhap,brHangMua,brTonKho,brPhanCap,brDuPhong,bsTenCongTrinh,brNgay,brSoNguoi,brChiTaiKhoBac,brChiTaiNganh=0,brChiTapTrung,iSTT=1
                                FROM DTBS_ChungTuChiTiet_PhanCap
                                WHERE  iTrangThai=1 AND iID_MaChungTu=@iID_MaChungTu
                                    ...
                                    SELECT iID_MaMucLucNganSach,iID_MaMucLucNganSach_Cha,bLaHangCha,sXauNoiMa,sTenCongTrinh, sLNS,sL,sK,sM,sTM,sTTM,sNG,sTNG,sMoTa,iID_MaChungTuChiTiet,iID_MaDonVi,sTenDonVi,sTenDonVi as sTenDonVi_BaoDam,iID_MaPhongBanDich,iID_MaPhongBan
                                    ,rNgay,rSoNguoi,rChiTaiKhoBac,rTonKho,rTuChi,rChiTaiNganh,rChiTapTrung,rHangNhap,rHangMua,rHienVat,rDuPhong,rPhanCap,sGhiChu,sMaCongTrinh
                                    ,brTuChi,brHienVat,brHangNhap,brHangMua,brTonKho,brPhanCap,brDuPhong,bsTenCongTrinh,brNgay,brSoNguoi,brChiTaiKhoBac,brChiTaiNganh=0,brChiTapTrung,iSTT=2
                                    FROM DTBS_ChungTuChiTiet
                                    WHERE iTrangThai=1 AND iID_MaChungTu=@iID_MaChungTu

                                    ORDER BY iSTT,sLNS, sM,sTM,sTTM,sNG, iID_MaDonVi");
            cmd.CommandText = SQL;
            cmd.Parameters.AddWithValue("@iID_MaChungTu", maChungTu);
            vR = Connection.GetDataTable(cmd);
            if (vR.Rows.Count > 0)
...
,rTuChi=SUM(rTuChi)
 ...
 ");
            ...
            string sMaMucLucNganSach, string sXauMaCacHang, string sXauCacHangDaXoa, string sXauMaCacCot, string sXauGiaTriChiTiet)
        {
            ...
            string[] arrHangGiaTri = sXauGiaTriChiTiet.Split(new string[] { BangDuLieu.DauCachHang }, StringSplitOptions.None);
            string maChungTuChiTiet;

            //Luu cac hang sua
            string[] arrHangThayDoi = sXauDuLieuThayDoi.Split(new string[] { BangDuLieu.DauCachHang }, StringSplitOptions.None);
                ...
                if (arrMaHang[i] != "")
                {
                    ...
                    else
                    {
                        ...
                        if (coThayDoi)
                        {
                            ...
                            bang.CmdParams.CommandTimeout = (int)TimeSpan.FromMinutes(2).TotalMilliseconds;
                            ...
                            if (maChungTuChiTiet == "") { 
                                ...
                                else if ("06,07,08,10,17".Contains(data["iID_MaPhongBan"].ToString()))
                                    bang.CmdParams.Parameters["@iID_MaPhongBanDich"].Value = data["iID_MaPhongBan"];                                                             
                            }

                            if (maChungTuChiTiet != ""  || !string.IsNullOrEmpty(bang.CmdParams.Parameters["@iID_MaPhongBanDich"].Value.ToString()))
                                bang.Save();                           
                        }
                    }
                }
