...
namespace VIETTEL.Models
{
    public class ReportModels
    {
        ...
        public static String DieuKien_NganSach_KhongDV(String MaND)
        {
            ...
            DK = String.Format(@" AND (iNamLamViec={0} AND iID_MaNamNganSach={1})  "
                               , iNamLamViec, iID_MaNamNganSach);
            return DK;
        }
        ...
        public static FlexCelReport LayThongTinChuKy(FlexCelReport fr, String ControllerName, String MaND = "", Boolean HienThi = true)
        {
            if (string.IsNullOrWhiteSpace(MaND))
                MaND = HttpContextHelper.Current.User.Identity.Name;
            DataTable dt = DanhMuc_BaoCao_ChuKyModels.Get_dtLayThongTinChuKy(ControllerName, MaND);
        ...
        public static String LayMaPhongBan(String MaND)
        {
        ...
        public static String LayMoTa(String sLNS, string user = "")
        {
            String sMoTa = "";

            String SQL = String.Format(@"SELECT sMoTa FROM NS_MucLucNganSach WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec AND sLNS={0} AND sL=''", sLNS);
            SqlCommand cmd = new SqlCommand(SQL);
            cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(user));
            sMoTa = Connection.GetValue(cmd, "").ToString();
            return sMoTa;
        }
        public static String LayMoTa(DataRow dr, string[] arrNoiMa, string user)
        {
            String sMoTa = "", sXauNoiMa = "";
            for (int i = 0; i < arrNoiMa.Length; i++)
            {
                if (sXauNoiMa == "")
                {
                    sXauNoiMa += dr[arrNoiMa[i]];
                }
                else
                {
                    sXauNoiMa += "-" + dr[arrNoiMa[i]];
...
SELECT sMoTa FROM NS_MucLucNganSach WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec AND sXauNoiMa={0}", sXauNoiMa);
            SqlCommand cmd = new SqlCommand(SQL);
            cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(user));
            sMoTa = Connection.GetValue(cmd, "").ToString();
            return sMoTa;
        }
        public static void numberCount(DataTable dt)
        {
        ...
        public static DataTable GetDtNganh(string nam)
        {  
            SqlCommand cmd = new SqlCommand();          

            cmd.CommandText = @"select sNG, sMoTa from NS_MucLucNganSach where iTrangThai = 1 and iNamLamViec = @nam and sLNS = ''";
