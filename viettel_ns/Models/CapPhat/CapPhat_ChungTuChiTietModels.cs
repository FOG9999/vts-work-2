...
namespace VIETTEL.Models
{
    public class CapPhat_ChungTuChiTietModels : CapPhatChungTuChiTietCha
    {
        ...
        private static void LayChiTieu(String iID_MaCapPhat, DataTable vR)
        {
            ...
            var dkDonVi = new NganSachService().GetDonviListByUser(HttpContextHelper.Current.User.Identity.Name, data["iNamLamViec"])
                                                       ...
                                                       )
        {
            ...
            String iID_MaPhongBanQuanLy = DonViModels.getPhongBanCuaDonVi(iID_MaDonVi, HttpContextHelper.Current.User.Identity.Name);
...
", strTruong, DK, iID_MaPhongBanQuanLy, DKMucCap);
        ...
        public static DataTable LayDtChungTuChiTietDMNB(string iID_MaCapPhat, Dictionary<string, string> arrGiaTriTimKiem, string MaND)
        {
            ...
            if (!string.IsNullOrEmpty(sDSLNS))
            {
                ...
                {
                    dkDonVi = new NganSachService().GetDonviListByUser(HttpContextHelper.Current.User.Identity.Name, iNamLamViec)
                        .ToDictionary(x => x.iID_MaDonVi, x => x.iID_MaDonVi + " - " + x.sTen).Select(x => x.Key).Join();
                }
        ...
        {
            var loai = HttpContextHelper.Current.Request.GetQueryStringValue("f", 0);
