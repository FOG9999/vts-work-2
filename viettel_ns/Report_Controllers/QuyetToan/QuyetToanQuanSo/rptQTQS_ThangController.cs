...
{
    public class rptQTQS_ThangController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        public Microsoft.AspNetCore.Mvc.ActionResult ViewPDF(string MaND, string iID_MaPhongBan, bool bBq, string iTuThang, string iDenThang,
                                    string iID_MaDonVi)
        {
            HamChung.Language();
            ...
            if (bBq)
            {
                ...
                for (int i = Convert.ToInt32(iTuThang); i <= Convert.ToInt32(iDenThang); i++)
                {
                   ...
                   DKPhongBan = ThuNopModels.DKPhongBan_QuyetToan(MaND, cmd);
                    ...
                    {
                        DK1 += "iID_MaDonVi=@iID_MaDonVia" + j;
                        if (j < arrDonVi.Length - 1)
                            DK1 += " OR ";
                        cmd.Parameters.AddWithValue("@iID_MaDonVia"+j,arrDonVi[j]);
                    }
                    if (String.IsNullOrEmpty(DK1)) DK1 = "1=1";
                    String SQL = String.Format(@"SELECT COUNT(*) FROM QTQS_ChungTu WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec AND iThang_Quy=@iThang {0} {1} AND ({2})", DKDonVi, DKPhongBan,DK1);
                    cmd.CommandText = SQL;
                    cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(MaND));
                    cmd.Parameters.AddWithValue("@iThang", i);
...
");
                ...
                where A.iTrangThai=1 and ((A.iNamLamViec=@iNamLamViec  AND iThang_Quy<=@Thang2 ) OR  A.iNamLamViec<@iNamLamViec) AND B.iNamLamViec_DonVi=@iNamLamViec
                {0} {1} {2} {3}
...
(
                ...
                for (int j = 0; j < dt.Columns.Count; j++)
                {
                    ...
                    for (int c = 0; c < dt.Rows.Count; c++)
                    {
                        ...
                        if (dt.Columns[j].ColumnName.StartsWith("r"))
                        {
                            ...
                            iBinhQuan = Math.Round(iBinhQuan / data.Count, 0, MidpointRounding.AwayFromZero);
            ...
            else
            {
                for (int i = Convert.ToInt32(iTuThang); i <= Convert.ToInt32(iDenThang); i++)
                {
                    ...
                    DKDonVi = ThuNopModels.DKDonVi(MaND, cmd);
                    DKPhongBan = ThuNopModels.DKPhongBan_QuyetToan(MaND, cmd);
                    String SQL = String.Format(@"SELECT COUNT(*) FROM QTQS_ChungTu WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec AND iThang_Quy=@iThang {0} {1} AND iID_MaDonVi=@iID_MaDonVi", DKDonVi, DKPhongBan);
                    cmd.CommandText = SQL;
                    cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(MaND));
                    cmd.Parameters.AddWithValue("@iThang", i);
                    cmd.Parameters.AddWithValue("@iID_MaDonVi", iID_MaDonVi);
...
");
                ...
                where iTrangThai=1 and ((iNamLamViec=@iNamLamViec  AND iThang_Quy<=@Thang ) OR  iNamLamViec<@iNamLamViec) and iID_MaDonVi=@MaDV  {0} {2}
