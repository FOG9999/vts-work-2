...
// cac bien __Init dùng để lưu lại giá trị nguyên thủy trước khi bị modify, sử dụng các gí trị này làm giá trị trước điều chỉnh, trước khi lưu
var arrChiPhiSave = [], arrChiPhiInit = [];
var arrNguonVonSave = [], arrNguonVonInit = [];
var arr_NguonVon_Dropdown = [];
var arrHangMucChuTruong = [];
var arrHangMucSave = [], arrHangMucInit = [];
var arrHangMucTemp = [];
...
function GetChiPhiHangMucTheoQDDauTu() {
    ...
    $.ajax({
        ...
        type: "POST",
        data: { qdDauTuId: qdDauTuId, isDieuChinh: true },
        dataType: "json",
        cache: false,
...
function updateGiatriTruocDieuChinhTruocKhiLuu() {
    arrNguonVonSave.forEach((nv, index) => {
        var oldNguonVon = arrNguonVonInit.find(old => old.id === nv.id)
        if (oldNguonVon && oldNguonVon.fTienPheDuyet !== nv.fTienPheDuyet) {
            arrNguonVonSave[index].fGiaTriTruocDieuChinh = oldNguonVon.fTienPheDuyet;
        } 
    });

    arrChiPhiSave.forEach((cp, index) => {
        var oldCp = arrChiPhiInit.find(old => old.id === cp.id)
        if (oldCp && oldCp.fTienPheDuyet !== cp.fTienPheDuyet) {
            arrChiPhiSave[index].fGiaTriTruocDieuChinh = oldCp.fTienPheDuyet;
        }
    })

    arrHangMucSave.forEach((hm, index) => {
        var oldHm = arrHangMucInit.find(old => old.id === hm.id);
        if (oldHm && oldHm.fTienPheDuyet !== hm.fTienPheDuyet) {
            arrHangMucSave[index].fGiaTriTruocDieuChinh = oldHm.fTienPheDuyet;
        }
    })
}
...
function GetFormDataQuyetDinhDauTu() {
    ...
    return {
        ...
        fTongMucDauTuPheDuyet: giaPheDuyet,
        sDiaDiem: diaDiem,
...
async function Luu() {
    ...
    updateGiatriTruocDieuChinhTruocKhiLuu();

    $.ajax({
        type: "POST",
        url: "/QLVonDauTu/QLPheDuyetDuAn/Save",
        data: { model: item, isDieuChinh: true, fTongMucPheDuyetTheoChuTruong: $("#fTongMucPheDuyetTheoChuTruong").val() },
        dataType: "json",
        cache: false,
        success: function (resp) {
...
function LoadDataViewChiPhi() {
    ...
    arrChiPhiSave.forEach(x => {
        var fontWeight = x.iID_ChiPhi_Parent ? "" : "font-weight: bold;";
        ...
        html += `<td style=" ${fontWeight}text-align:left;" ><input type = "text" onblur="UpdateChiPhi(this)" class="form-control r_TenChiPhi" value = "` + x.sTenChiPhi + '" /> </td> <input type="hidden" class="r_iID_DuAn_ChiPhiID" value=""/> <input type="hidden" class="r_iID_ChiPhiParentID" /> <input type="hidden" class="r_iID_ChiPhiID" value = "' + x.iID_ChiPhiID + '"/>';
...
// disable input chi phi if chi phi has any hang muc
function onHideModalHangMuc() {
    $("#modal-listdetailchiphi").on('hidden.bs.modal', () => {
        arrChiPhiSave.forEach(cp => {
            var hangmucs = arrHangMucSave.filter(hm => hm.iID_DuAn_ChiPhi === cp.iID_DuAn_ChiPhi);
            if (hangmucs.length > 0) {
                $("#tblChiPhiDauTu *[data-id='" + cp.id + "']").find('input').prop("disabled", true);
            }
        })
    })
}
...
function DetailChiPhi(nutDetail) {
    // clone ra mảng mới và modify trên mảng đó
    arrHangMucTemp = JSON.parse(JSON.stringify(arrHangMucSave))
    ...
    var arrHangMucByChiPhi = arrHangMucTemp.filter(x => x.iID_DuAn_ChiPhi == iIdDuAnChiPhi);
...
function LoadViewHangMuc(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].isDelete == true) {
            var parent = data[i].iID_ParentID ? "child" : "parent";
            ...
            dongMoi += "<td class='r_HanMucDauTu' align='right'><input type='text' onblur='UpdateHangMuc(this)' disabled style='text-align: right' class='form-control txtHanMucDauTu' onkeyup='ValidateNumberKeyUp(this);' onkeypress='return ValidateNumberKeyPress(this, event);' value='" + FormatNumber(data[i].fTienPheDuyet) + "'/></td>";
            ...
            var parent = data[i].iID_ParentID ? "" : "parent";
            ...
            dongMoi += "<td class='r_HanMucDauTu' align='right'><input type='text' style='text-align: right' disabled class='form-control txtGiaTriTruocDieuChinh' value='" + FormatNumber(data[i].fGiaTriTruocDieuChinh) + "'/></td>";
            dongMoi += "<td class='r_HanMucDauTu' align='right'><input type='text' onblur='UpdateHangMuc(this)' style='text-align: right' class='form-control txtHanMucDauTu' onkeyup='ValidateNumberKeyUp(this);' onkeypress='return ValidateNumberKeyPress(this, event);' value='" + FormatNumber(data[i].fTienPheDuyet) + "'/></td>";
            dongMoi += "<td align='center' class='width-200'>";
            dongMoi += "<button class='btn-add-child btn-icon' type = 'button' onclick = 'ThemMoiHangMucCon(this)' > " +
                "<i class='fa fa-plus fa-lg' aria-hidden='true'></i>" +
                "</button> ";
...
function saveHangMuc() {
    ...
    arrHangMucSave = JSON.parse(JSON.stringify(arrHangMucTemp));
    ...
    // tính toán lưu lại tổng giá trị các hạng mục cho chi phí hiện tại
...
function UpdateHangMuc(nutCreateHangMuc) {
    ...
    var objHangMuc = arrHangMucTemp.filter(x => x.id == id)[0];
    arrHangMucTemp = arrHangMucTemp.filter(function (x) { return x.id != id });
...
function ShowHideButtonHangMuc() {
    arrHangMucTemp.forEach(x => {
        var countChild = arrHangMucTemp.filter(y => y.iID_ParentID == x.iID_QDDauTu_DM_HangMucID);
        if (countChild.length > 0) {
            ...
            $("#tblHangMucChinh *[data-id='" + x.id + "']").find(".txtTenHangMuc, select, .txtHanMucDauTu").removeAttr('disabled');
        }
    })
}
...
// trim space for all text input
function trimspaceText(target) {
    let originalVal = $(target).val();
    $(target).val(originalVal.trim());
}

$("input[type=text]").blur((e) => {
    trimspaceText(e.target);
})
...
function checkExistSoQuyetDinh() {
    ...
    return $.ajax({
        url: "/QLVonDauTu/QLPheDuyetDuAn/CheckExistSoQuyetDinh",
        type: "GET",
