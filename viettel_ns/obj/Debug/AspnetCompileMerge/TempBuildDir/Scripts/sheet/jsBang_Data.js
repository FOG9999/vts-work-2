...
var Bang_DoRongHang = 41; // Giá trị xác định độ rộng cố định của 1 hàng
...
var Bang_sMauSac_TuChoi = "#ffffff";
...
function Bang_KhoiTao() {
    ...
    if (document.getElementById('idBangChiDoc') != null &&
        ...
        console.log("set Bang_ChiDoc:" + false);
        Bang_ChiDoc = false;
    }
...
function Bang_Ready() {
    $('#' + BangDuLieuID_Slide + ' td').each(function () {
        Bang_keys.event.action(this, function (nCell) {
            ...
            // neu dong bi xoa thi bo qua
            if (Bang_arrHangDaXoa[h0] == true) {
                return;
            }
...
function Bang_HamTruocKhiKetThuc(iAction) {
    ...
    console.log("Bang_ChiDoc:" + Bang_ChiDoc);
    ...
    for (i = 0; i < Bang_nH; i++) {
        ...
        for (j = 0; j < Bang_nC; j++) {
            ...
            if (Bang_arrThayDoi[i][j]) {
                ...
                // set giá trị cho các trường đang k lấy được giá trị trong vm (SheetEditViewModel)
    ...
    for (var h = hMin; h < hMax; h++) {
        ...
        if (Bang_arrHangDaXoa[h]) {
            ...
            if (c_sMauSac != null && c_sMauSac >= 0) {
                ...
                //neu mau sac do tu choi boi dong fixed
                if (bgcolor == "#FF0000") {
...
function Bang_SetPosition(y) {
    if (Bang_Viewport_N == 0) {
        // update status scrolling
        scrolling = false;
        return;
    }

    console.log("Bang_Viewport_hMin:" + Bang_Viewport_hMin);


    if (Bang_Viewport_N == 0) return;
    var yMin = Bang_Viewport_hMin * Bang_DoRongHang;
    var yMax = yMin + 20 * Bang_DoRongHang;
    if (yMin <= y && y <= yMax) {
        // update status scrolling
        scrolling = false;
        return;
    ...
    if (Bang_keys.fnSetViewportPosition(cs)) {
        // scroll table content
        $('#' + Bang_ID_TB11_Div).scrollTop(y);
        // update status scrolling
        scrolling = false;
    }
    else
