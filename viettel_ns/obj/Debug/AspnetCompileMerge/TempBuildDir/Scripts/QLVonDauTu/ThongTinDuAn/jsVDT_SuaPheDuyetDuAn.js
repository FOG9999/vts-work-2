...
function Luu() {
    ...
    */
    ...
    var objectId = $("#inputFile_ObjectID").val();
    UploadFile(objectId, 103);
}
...
function CalculateDataChiPhi(itemId) {
    ...
    if (objItem.iID_ChiPhi_Parent != null && objItem.iID_ChiPhi_Parent != "") {
        ...
        if (objParentItem != null) {
            var arrChildSameParent = arrChiPhiSave.filter(function(x) {
                return x.iID_ChiPhi_Parent == objItem.iID_ChiPhi_Parent && x.iID_ChiPhi_Parent != ""
            });
        ...
        objItem.fTienPheDuyet = parseInt($('*[data-id="' + objItem.iID_DuAn_ChiPhi + '"]').find('.txtGiaTriChiPhi').val().replaceAll('.',''));
        let objIndex = arrChiPhiSave.map(it => it.iID_DuAn_ChiPhi).indexOf(objItem.iID_DuAn_ChiPhi);
        arrChiPhiSave[objIndex] = objItem;
    }
}
...
function GetListDataChiPhiDefaultAjax() {
    ...
    return $.ajax({
        ...
        cache: false,
        success: function (data) {
            SetListDataChiPhiDefaultAjax(data);
        }
    });
}
...
function DetailChiPhi(nutDetail) {
    ...
    var arrHangMucByChiPhi = arrHangMucTemp.filter(x => x.iID_DuAn_ChiPhi == iIdDuAnChiPhi && !x.isDelete);
...
function LoadViewHangMuc(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].iID_ParentID == null) {
            ...
            dongMoi += "<tr style='cursor: pointer;' class='child' data-id='" + data[i].id + "' data-iidduanchiphi='" + data[i].iID_DuAn_ChiPhi + "'>";
        }
...
function GetHangMucTheoQDDauTu() {
    var qdDauTuId = $("#iIDQuyetDinhDauTuId").val();
    arrHangMucSave = [];
    $.ajax({
...
function ShowHideButtonHangMuc() {
    arrHangMucTemp.forEach(x => {
        var countChild = arrHangMucTemp.filter(y => y.iID_ParentID == x.iID_QDDauTu_DM_HangMucID && (y.isDelete == null || y.isDelete == false));
        if (countChild.length > 0) {
...
function GetIndexRowNewChild(parentId) {
    var listChild = arrHangMucSave.filter(x => x.iID_ParentID == parentId);
    if (listChild.length == 0)
        return;
}

function CalculateDataHangMucByChiPhi(itemId) {
    var objItem = arrHangMucTemp.find(x => x.id == itemId);
    ...
    var arrChildSameParent = arrHangMucTemp.filter(function (x) { return x.iID_ParentID == objItem.iID_ParentID && x.iID_ParentID != "" });
    if (arrHasValue(arrChildSameParent)) {
...
function CalculateTotalParentHangMuc(objParentItem, arrChild) {
    var sumNotDeleted = 0; // tổng các con bị xóa
    arrChild.forEach(x => {
        if (!x.isDelete)
            sumNotDeleted += x.fTienPheDuyet;
    });

    //giá trị của bố  = giá trị bố hiện tại - giá trị các con bị xóa
    arrHangMucTemp = arrHangMucTemp.filter(x => x.id != objParentItem.id);
    objParentItem.fTienPheDuyet = sumNotDeleted;
    $("#tblHangMucChinh [data-id='" + objParentItem.id + "']").find('.txtHanMucDauTu').val(FormatNumber(sumNotDeleted));
...
function ThemMoiNguonVon() {
    ...
    dongMoi += "<td class='r_GiaTriNguonVon' ><input type='text' disabled style='text-align: right' class='form-control txtGiaTriPheDuyetCTDT'/></td>"
...
function UpdateNguonVon(nutCreate) {
    ...
    // tìm index của phần tử bị thay đổi
    var indexUpdateElement = arrNguonVonSave.map(e => e.id).indexOf(id);
    // thay đổi giá trị
    arrNguonVonSave[indexUpdateElement].fTienPheDuyet = giaTriPheDuyet;
    arrNguonVonSave[indexUpdateElement].iID_NguonVonID = iID_NguonVonID;
...
function LoadDataViewNguonVon() {
    if (arrNguonVonSave != null) {
        for (var i = 0; i < arrNguonVonSave.length; i++) {
            ...
            dongMoi += "<td class='r_GiaTriNguonVon'><input type='text' disabled style='text-align: right' class='form-control txtGiaTriPheDuyetCTDT' value='" + FormatNumber(arrNguonVonSave[i].fTienPheDuyetCTDT) + "'/></td>"
...
function XoaDong(nutXoa, idBang) {
    ...
    if (idBang == TBL_CHI_PHI_DAU_TU) {
        ...
        var objXoa = arrHangMucTemp.filter(function (item) { return item.id == id; })[0];
        objXoa.isDelete = !objXoa.isDelete;
        arrHangMucTemp = arrHangMucTemp.filter(function (item) { return item.id != id });
        arrHangMucTemp.push(objXoa);
        if (checkXoa == 1) {
            $(dongXoa).find('input, select').prop("disabled", "");
        } else {
            $(dongXoa).find('input, select').prop("disabled", "disabled");
