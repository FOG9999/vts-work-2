...
{
    public class DuToanBSPhanCapChungTuChiTiet2Controller : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        private const string VIEW_PHANCAP_CHITIET_INDEX = "PhanCap_ChungTuChiTiet_Index2.aspx";
        private const string VIEW_PHANCAP_CHITIET_FRAME = "PhanCap_ChungTuChiTiet_Index_DanhSach_Frame2.aspx";
        ...
        /// <returns></returns>
        [Authorize]
        public Microsoft.AspNetCore.Mvc.ActionResult ChungTuChiTietFrame(String sLNS, String MaLoai)
        {
            //Kiểm tra quyền của người dùng với chức năng
            if (BaoMat.ChoPhepLamViec(User.Identity.Name, BANG_CHUNGTU_CHITIET, DETAIL) == false)
            {
        ...
        public Microsoft.AspNetCore.Mvc.ActionResult CapNhatChungTuChiTiet(String ChiNganSach, String iID_MaChungTu, String sLNS)
        {
            DataTable dtCT = DuToanBSChungTuChiTietModels.LayDanhSachChungTuChiTiet2(iID_MaChungTu);
            DataRow data = dtCT.NewRow();
            ...
            for (int i = 0; i < arrMaHang.Length; i++)
            {
                if (arrMaHang[i] != "")
                {
                    ...
                    {
                        bool check = false;
                        if (arrMaHang[i].Split('_')[2].StartsWith("2") || arrMaHang[i].Split('_')[2].Substring(0, 7) == "4050000")
                        {
                            if (arrMaHang[i].Split('_')[2] == Convert.ToString(dtCT.Rows[j]["sXauNoiMa"]))
                            {
                                check = true;
                            }
                        }
                        ...
                        {
                            if (arrMaHang[i].Split('_')[2].Substring(8) == Convert.ToString(dtCT.Rows[j]["sXauNoiMa"]).Substring(8))
                            {
                                check = true;
                            }
                        }
                    ...
                    if (iKyThuat == "1" && MaLoai != "2")
                    {
                        ...
                        if (arrMaHang[i] != "")
                        {
                            ...
                            var Xau = dtMucLuc.Rows[0]["sXauNoiMa"].ToString();

                            if (DuToanBSChungTuChiTietModels.KiemTraDonViPhanCap2Lan(iID_MaDonVi) && Xau.EndsWith("010-011-6950-6954-10-23"))
                            {
                            ...
                            else
                            {
                                ...
                                if (okCoThayDoi)
                                {
                                    ...
                                    if (iID_MaCapPhatChiTiet == "")
                                    {
                                        ...
                                        bang.CmdParams.Parameters.AddWithValue("@iID_MaChungTu", data["iID_MaChungTuChiTiet"]);
        ...
        [Authorize]
        [AcceptVerbs(HttpVerbs.Post)]
        public Microsoft.AspNetCore.Mvc.ActionResult SearchSubmit(String ParentID, String iID_MaChungTu, string iLoai)
        {
