...
{
    public class rptNCSKT_TK08ViewModel
    {
        public SelectList PhongBanList { get; set; }
        public SelectList NganhList { get; set; }
    }
    public class rptNCSKT_TK08Controller : FlexcelReportController
    {
        public string _viewPath = "~/Areas/SKT/Views/Reports/TrinhKy/";
        private string _filePath = "~/Areas/SKT/FlexcelForm/TrinhKy/rptNCSKT_TK08/rptNCSKT_TK08.xls";
        //private string _filePath_ncskt = "~/Areas/SKT/FlexcelForm/TongHop/rptNCSKT_TK08/rptNCSKT_TK08_ncskt.xls";
        ...
        public Microsoft.AspNetCore.Mvc.ActionResult Index()
        {
            if (User.Identity.Name.EndsWith("b2"))
            {
                ...
                var vm = new rptNCSKT_TK08ViewModel
                {
                    NganhList = nganhList,
                    PhongBanList = phongbanList.ToSelectList("sKyHieu", "sMoTa", "-1", "-- Chọn phòng ban --"),
                };

                var view = _viewPath + "rptNCSKT_TK08.cshtml";
        ...
        private Dictionary<bool, ExcelFile> CreateReport()
        {
            ...
            else
            {
                ...
                fr.SetValue(new
                {
                    ...
                    h2 = nganh.IsEmpty() ? $"(Tổng hợp ngành - {_nganSachService.GetPhongBanById(_id_phongban).sMoTa})" : $"Ngành: {nganh}",
        ...
        private DataTable GetData()
        {
            ...
            using (var cmd = new SqlCommand("sp_ncskt_report_tk08", conn))
            {
                cmd.AddParams(new
                {
