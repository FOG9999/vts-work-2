...
namespace VIETTEL.Areas.QLNH.Controllers.BaoCaoThongKe
{
    public class BaoCaoSoChuyenNamSauController : FlexcelReportController
    {
        ...
        [HttpPost]
        public Microsoft.AspNetCore.Mvc.ActionResult GetModalInBaoCao(int slbNamKeHoach, Guid? slbDonVi)
        {
        ...
        public Microsoft.AspNetCore.Mvc.ActionResult ExportFile(string txtTieuDe1, string txtTieuDe2, Guid? txtIdDonVi, string txtSTenDonVi, int txtNamKeHoach, int? slbDonViUSD, int? slbDonViVND, string ext = "xlsx", int to = 1)
        {
            string fileName = string.Format("{0}.{1}", "Bao cao chi tiet so chuyen nam sau nam" + txtNamKeHoach, ext);
            ExcelFile xls = TaoFileExel(txtTieuDe1, txtTieuDe2, txtIdDonVi, txtSTenDonVi, txtNamKeHoach, slbDonViUSD, slbDonViVND, to);
            return Print(xls, ext, fileName);
        }
        public ExcelFile TaoFileExel(string txtTieuDe1, string txtTieuDe2, Guid? iIdDonVi, string sTenDonVi, int iNamKeHoach, int? slbDonViUSD, int? slbDonViVND, int to = 1)
        {
        ...
        [HttpPost]
        public Microsoft.AspNetCore.Mvc.ActionResult TimKiem(Guid? iDonVi, int? iNamKeHoach)
        {
            NH_QT_QuyetToanNienDo_ChiTietView vm = new NH_QT_QuyetToanNienDo_ChiTietView();
            List<NS_DonVi> lstDonViQL = _qlnhService.GetDonviList(PhienLamViec.NamLamViec).ToList();
            lstDonViQL.Insert(0, new NS_DonVi { iID_Ma = Guid.Empty, sTen = "--Chọn đơn vị--" });
