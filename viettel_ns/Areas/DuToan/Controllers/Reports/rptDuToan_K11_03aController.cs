...
{
    public class rptDuToan_K11_03aController : FlexcelReportController
    {
        #region var def
        ...
        public JsonResult Ds_To(string id_phongban, string id_nganh, int loaiBaoCao)
        {
            ...
            var nganhs = data.SelectDistinct("nganh", "Nganh,sM");
            var nganhsTong = data.SelectDistinct("nganh", "Nganh");

            return ds_ToIn(nganhs.Rows.Count == 0 ? 0 : nganhs.Rows.Count + (nganhsTong.Rows.Count * 2) + 1, _columnCount);
        }
        ...
        private ExcelFile createReport()
        {
            ...
            fr.SetValue(new
            {
                ...
                TieuDe2 = $"{tenNganh} - { (_loaiBaoCao == -1 ? "Tổng hợp" : (_loaiBaoCao == 1 ? "Tự chi" : "Phân cấp đơn vị"))}",
        ...
        private void loadData(FlexCelReport fr)
        {
            ...
            var dtX = dt.SelectDistinct("Nganh", "sLNS,sNG,TenNganh,sM,sXauNoiMa,Loai");
            ...
            for (int i = 0; i < rowsCountX; i++)
            {
                ...
                row_hienvat["sNG"] = dtNganh.Rows[i]["sNG"];
                row_hienvat["TenNganh"] = dtNganh.Rows[i]["TenNganh"];
                row_hienvat["Loai"] = 2;
                dtX.Rows.Add(row_hienvat);
            }
            ...
            //var dtMuc = getMucMoTa(PhienLamViec.iNamLamViec);
            ...
            for (int i = 0; i < _columnCount; i++)
            {
                ...
                if (i < columns.Count)
                {
                    ...
                    var sM = row.Field<string>("sM");
                    if (!string.IsNullOrWhiteSpace(sM))
                    {
                        //sM = row.Field<string>("sLNS") + " - " + sM + Environment.NewLine + Environment.NewLine +
                        //    mlns.FirstOrDefault(x => x.sXauNoiMa == row.Field<string>("sXauNoiMa")).sMoTa;
                        sM = sM + Environment.NewLine + Environment.NewLine +
                            mlns.FirstOrDefault(x => x.sXauNoiMa == row.Field<string>("sXauNoiMa")).sMoTa;
        ...
        private DataTable getTable(string nam, string id_phongban, string id_nganh, int loaiBaoCao, string username)
        {
            ...
            var sql = FileHelpers.GetSqlQuery("rptDuToan_K11_03a.sql");
