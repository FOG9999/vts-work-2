...
{
    public class rptDuToan_Kiem_A3Controller : FlexcelReportController
    {
        #region var def
        private string _filePath = "~/Report_ExcelFrom/DuToan/DonVi/rptDuToan_Kiem_A3.xls";
        ...
        {
            var phongBanList = _nganSachService.GetPhongBansQuanLyNS("05,06,07,08,10");
            ...
            return View(@"~/Areas\DuToan\Views\Report\Kiem\rptDuToan_Kiem_A3.cshtml", vm);
        }
        ...
        private Dictionary<bool, FlexCelReport> loadData(FlexCelReport fr)
        {
            ...
            {
                var data = dt.SelectDistinct("ChiTiet", "sLNS,sL,sK,sM,sTM,sTTM,sNG,sXau,sMoTa");
                var dtX = dt.SelectDistinct("X", "iID_MaDonVi,TenDonVi");
                        ...
                        data.AsEnumerable()
                           ...
                           .ForEach(r =>
                           {
                               var value1 = dt.AsEnumerable()
                                       .ToList()
                                       .Where(x => (x.Field<string>("iID_MaDonVi") == row.Field<string>("iID_MaDonVi")) &&
                                                    x.Field<string>("sXau") == r.Field<string>("sXau"))
        ...
        {
            #region get data
            var donvis = (',' + _DTService.Get_DonViDT_TheoBql_ExistData_Negative(PhienLamViec.iNamLamViec, PhienLamViec.iID_MaDonVi, PhienLamViec.iID_MaPhongBan == "11" || PhienLamViec.iID_MaPhongBan == "02" ? "05,06,07,08,10" : PhienLamViec.iID_MaPhongBan, _id_phongban).AsEnumerable().Select(x => x.Field<string>("Id")).Join(","))
                                .Split(',').Skip((_to - 1) * (_columnCount)).Take(_columnCount).Join(",");
            using (var conn = ConnectionFactory.Default.GetConnection())
            using (var cmd = new SqlCommand("DT_report_kiem_a3", conn))
            {
        ...
        public JsonResult Ds_To(string id_PhongBan)
        {
            ...
            var count = _DTService.Get_DonViDT_TheoBql_ExistData_Negative(PhienLamViec.iNamLamViec, PhienLamViec.iID_MaDonVi, PhienLamViec.iID_MaPhongBan == "11" || PhienLamViec.iID_MaPhongBan == "02" ? "05,06,07,08,10" : PhienLamViec.iID_MaPhongBan, id).Rows.Count;
